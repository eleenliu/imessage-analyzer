...
** Note: This application is only compatible with MacBooks and iMacs **
"""
...
def style(pal='husl'):            # stylize charts
    sns.set()
    sns.set_palette(pal,10)


def load_contacts():        # load contacts from Mac database
    ...
    for count in range(0,len(contacts['ZFULLNUMBER'])):
        contacts['ZFULLNUMBER'][count] = contacts['ZFULLNUMBER'][count]\
...
def get_info():   # get phone number and name from user
...
def volume(df):     # show total iMessage volume
...
def word_cloud(df):      # create word cloud of iMessage history
    ...
    try:
        for x in df['text']:
            x += " "
            text += x
    except TypeError:           # pass by NoneTypes
        pass

    try:
        cloud = WordCloud(max_words=60, background_color="white", width=1600, height=800, stopwords=None).generate(text)
...
def time_series(df):        # charts frequency of iMessages by month
    print("Generating time series by month...")
    style()
    df["date_uct"] = df["date_uct"].astype("datetime64")
    month = df['date_uct'].dt.month
    plot = sns.countplot(x=month, data=df)
    plot.set(xlabel='Month', ylabel='Count', title='Frequency of iMessages by Month')
    plt.show()
